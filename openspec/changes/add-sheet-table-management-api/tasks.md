...
- [ ] 1.1 创建 `src/lib/sheets-client.ts`，封装 Google Sheets API v4 客户端初始化和认证逻辑（从现有 `route.ts` 的 `getCommonDbConfig` 提取并复用密钥文件路径判断）
- [ ] 1.2 在 `src/lib/sheets-client.ts` 中导出共享的 CORS 头常量和 `createResponse` 工具函数，供管理路由和现有 CRUD 路由共用
- [ ] 1.3 重构现有 CRUD 路由 `src/app/api/[doc_id]/[[...sheet_name]]/route.ts`，改为从 `sheets-client.ts` 导入共享逻辑
...
- [ ] 2.4 实现 DELETE 处理器：接收 `{ title }` 参数，先查找对应 `sheetId`，再调用 `spreadsheets.batchUpdate` 的 `deleteSheet` 请求删除
- [ ] 2.5 实现 OPTIONS 处理器和错误处理（名称冲突 409、不存在 404、参数缺失 400）
